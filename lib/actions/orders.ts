...
export async function completeOrderAction(formData: FormData): Promise<OrderResponse> {
  try {
    ...
    // Calcular total (simplificado, asumir items fijos por ahora)
    const total = 72000; // placeholder
    ...
    // Crear items (placeholder)
    const itemsToInsert = [
      { orderId: result[0].id, name: 'Producto 1', price: 24000 },
      { orderId: result[0].id, name: 'Producto 2', price: 24000 },
      { orderId: result[0].id, name: 'Producto 3', price: 24000 },
    ];
